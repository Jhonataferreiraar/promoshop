  ...
  async function generate() {
    ...
    try {
      ...
      const body = isTemplate
        ? { templateType: contentType, profileMode, profile, groupCode, bio, themeId, ctaText, showQrCode }
  ...
  return <div className="instagram-admin-layout personal-share-layout">
    <section className="panel instagram-settings personal-share-panel">
      ...
      <div className="settings-grid two-columns personal-share-form">
        ...
        {contentType === 'profile' && <label className="wide-field">Bio do perfil<textarea rows="3" value={bio} onChange={(event) => setBio(event.target.value)} /><small>Até três linhas curtas para manter a arte equilibrada.</small></label>}
      </div>
      ...
      <div className="settings-grid two-columns personal-share-form personal-share-identity">
        ...
        <label>Chamada principal<input value={ctaText} onChange={(event) => setCtaText(event.target.value)} /><small>Aparece no botão amarelo. O Instagram não cria botão clicável na imagem.</small></label>
      </div>
