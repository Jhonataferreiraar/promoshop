  ...
  return <div className="instagram-admin-layout personal-share-layout">
    <section className="panel instagram-settings personal-share-panel">
      ...
      <div className="settings-grid two-columns personal-share-form">
        ...
        {contentType === 'site' && <><label>Título da página<input value={siteTitle} onChange={(event) => setSiteTitle(event.target.value)} maxLength={80} /><small>Aparece em destaque no card.</small></label><label className="wide-field">Texto da página<textarea rows="5" value={siteDescription} onChange={(event) => setSiteDescription(event.target.value)} maxLength={300} /><small>Use até cinco linhas curtas para explicar o que a PromoShop oferece.</small></label></>}
        ...
        {contentType === 'profile' && <label className="wide-field">Bio do perfil<textarea rows="5" value={bio} onChange={(event) => setBio(event.target.value)} /><small>Até cinco linhas curtas. O template ajusta a fonte para evitar cortes.</small></label>}
      </div>
