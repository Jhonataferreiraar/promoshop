...
function ThemeSwitcher({ compact = false }) {
  ...
  const icon = preference === 'dark' ? 'theme-dark' : preference === 'light' ? 'theme-light' : 'theme-system';
  const options = [
  ...
  return <div ref={switcherRef} className={`theme-switcher ${compact ? 'theme-switcher-compact' : ''} ${open ? 'is-open' : ''}`}>
    ...
    >
      <span className="theme-switcher-icon" aria-hidden="true"><Icon name={icon} size={17} /></span>
    </button>
    {open && <div ref={menuRef} className="theme-switcher-menu" role="menu" aria-label="Opções de tema">
      ...
      >
        <span aria-hidden="true"><Icon name={option.icon} size={16} /></span>
      ...
      <section className="hero">
        <div className="container hero-grid">
          ...
          <div className="hero-art" aria-hidden="true">
            ...
            <div className="floating-card card-two"><span><Icon name="sparkles" size={16} /></span><strong>Novas ofertas</strong></div>
          </div>
        </div>
      </section>

      <section className="search-panel container" aria-label="Filtros de ofertas">
        <label className="search-control"><small>O que você procura?</small><span className="search-box"><span aria-hidden="true"><Icon name="search" size={17} /></span><input list="offer-suggestions" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Busque por produto ou marca" /><datalist id="offer-suggestions">{suggestions.map((item) => <option value={item.title} key={item.slug}>{item.store}</option>)}</datalist></span></label>
...
function AdminApp() {
  ...
  async function loadQueueDuplicateInfo() {
    setQueueDuplicateInfoLoaded(false);
    try {
      ...
      setQueueDuplicateInfoLoaded(true);
    } catch (error) {
  ...
  useEffect(() => {
    if (!token || !['overview', 'queue'].includes(tab)) return undefined;
  ...
  function clearQueueDuplicates() {
    ...
    setDialog({
      ...
      title: 'Limpar duplicatas da fila?',
      body: `${duplicateCount} duplicata(s) pendente(s) de ofertas serão marcadas como ignoradas. A cópia principal de cada produto será mantida. Ofertas ativas, links, itens enviados ou em publicação e filas do WhatsApp e do Instagram não serão alterados.`,
      ...
      onConfirm: async () => {
        ...
        try {
          ...
          const refreshes = [load(), loadQueueDuplicateInfo()];
          if (tab === 'queue') refreshes.push(loadQueuePage(true));
          await Promise.all(refreshes);
          setMessage(result.removed ? `${result.removed} duplicata(s) de ofertas foram retiradas da fila.` : 'Nenhuma duplicata pendente encontrada.');
        } catch (error) {
  ...
  return <div className="admin-shell"><aside><div className="sidebar-brand"><Logo name={data.config.brandName || 'PromoShop'} /><small>Painel administrativo</small></div><nav>{navGroups.map((group) => { const visibleItems = group.items.filter(canView); if (!visibleItems.length) return null; return <div className="nav-group" key={group.label}><span>{group.label}</span>{visibleItems.map((id) => <button className={tab === id ? 'active' : ''} key={id} onClick={() => setTab(id)}><i>{navIcons[id]}</i><span className="nav-label">{tabLabels[id]}</span>{id === 'inbox' && unreadInboxCount > 0 && <b className="nav-badge">{unreadInboxCount > 99 ? '99+' : unreadInboxCount}</b>}</button>)}</div>; })}</nav><div className="sidebar-footer"><a href="/">Ver site <span>↗</span></a><button className="logout" onClick={logout}>Sair</button></div></aside><main className={`admin-main ${!canEdit(tab) ? 'permission-readonly' : ''}`}><header><div><span className="eyebrow dark">CENTRAL DE CONTROLE</span><h1>{tabLabels[tab]}</h1><p>{tabDescriptions[tab]}</p></div><div className="header-actions"><ThemeSwitcher compact /><span className={`header-status ${whatsapp.status === 'connected' ? 'online' : whatsappConnecting ? 'pending' : ''}`}><i></i>WhatsApp {whatsapp.status === 'connected' ? 'ativo' : whatsappConnecting ? 'conectando' : 'inativo'}</span>{['overview', 'offers', 'sources'].includes(tab) && canEdit('sources') && <><button className="button primary" onClick={collect}>Atualizar ofertas</button><button className="button subtle" onClick={() => collect({ pauseRound: true })}>Pausar rodada e atualizar</button></>}</div></header>{message && <div className="toast" role="status"><span>{message}</span><button type="button" onClick={() => setMessage('')} aria-label="Fechar aviso">×</button></div>}{tab !== 'overview' && tab !== 'security' && !canEdit(tab) && <div className="permission-notice" role="status"><strong>Modo consulta</strong><span>Você pode visualizar esta área, mas não editar seus dados. Solicite ao administrador principal a liberação de edição.</span></div>}
    {tab === 'overview' && <div className="overview-layout"><section className="welcome-panel"><div><span className="eyebrow">RESUMO DA AUTOMAÇÃO</span><h2>{whatsapp.status === 'connected' ? 'Tudo pronto para publicar' : whatsappConnecting ? 'Finalizando a conexão' : 'WhatsApp precisa de atenção'}</h2><p>{whatsapp.status === 'connected' ? `O publicador está conectado a ${(data.config.whatsappGroups || []).length} grupo(s) e segue a agenda configurada.` : whatsappConnecting ? (whatsapp.message || 'O número foi vinculado e os grupos estão sendo sincronizados.') : 'Conecte o WhatsApp para que as ofertas da fila sejam enviadas automaticamente.'}</p></div><button className="button light" onClick={() => setTab('whatsapp')}>{whatsapp.status === 'connected' ? 'Ver configuração' : whatsappConnecting ? 'Acompanhar conexão' : 'Conectar WhatsApp'}</button></section><div className="stats"><div><span><i>◇</i>Ofertas no site</span><strong>{Number.isFinite(Number(data.publicOfferTotal)) ? Number(data.publicOfferTotal) : data.offers.filter((o) => o.status === 'active').length}</strong><small>Ativas e visíveis</small></div><div><span><i>↗</i>Na fila</span><strong>{Number(data.queueSummary?.pending || 0)}</strong><small>Aguardando publicação</small></div><div><span><i>✓</i>Enviadas</span><strong>{Number(data.queueSummary?.sent || 0)}</strong><small>Publicações concluídas</small></div><div><span><i>⌁</i>Fontes ativas</span><strong>{[data.config.enableMercadoLivre, data.config.enableShopee, data.config.enableAliexpress, data.config.enableMagalu].filter(Boolean).length}</strong><small>Coletas automáticas</small></div></div><section className="panel duplicate-overview-panel"><div className="panel-heading"><div><span className="section-step">INTEGRIDADE DA FILA</span><h2>Duplicatas pendentes</h2><p>Cópias repetidas ficam fora da publicação, sem alterar as ofertas ativas.</p></div><div className="duplicate-overview-actions"><button className="text-button" type="button" onClick={() => setTab('queue')}>Ver fila completa →</button>{canEdit('queue') && Number(queueDuplicateInfo?.duplicateCount || 0) > 0 && <button className="queue-clear-duplicates" type="button" disabled={queueDuplicateBusy} onClick={clearQueueDuplicates}>{queueDuplicateBusy ? 'Limpando…' : 'Limpar duplicatas'}</button>}</div></div><div className="duplicate-overview-grid"><div className="duplicate-total"><span><Icon name="database" size={18} /> Duplicatas detectadas</span><strong>{queueDuplicateInfoLoaded ? Number(queueDuplicateInfo.duplicateCount || 0).toLocaleString('pt-BR') : '—'}</strong><small>{queueDuplicateInfoLoaded ? `${Number(queueDuplicateInfo.duplicateGroupCount || 0).toLocaleString('pt-BR')} produto(s) com mais de uma cópia pendente` : 'Consultando a fila atual…'}</small></div><div className="duplicate-store-list"><div><strong>Por loja</strong><small>Somente cópias pendentes com a mesma origem ou link.</small></div>{queueDuplicateInfoLoaded && queueDuplicateInfo.stores?.length ? queueDuplicateInfo.stores.map((entry) => <div className="duplicate-store-row" key={entry.store}><span>{entry.store}</span><strong>{Number(entry.duplicateCount || 0).toLocaleString('pt-BR')}</strong></div>) : <p className="duplicate-empty">{queueDuplicateInfoLoaded ? 'Nenhuma duplicata pendente encontrada.' : 'Aguardando a leitura da fila.'}</p>}</div></div></section><section className="panel table-panel"><div className="panel-heading"><div><h2>Próximas publicações</h2><p>Itens que serão enviados primeiro.</p></div><button className="text-button" onClick={() => setTab('queue')}>Ver fila completa →</button></div><QueueTable queue={data.queue} /></section></div>}
    {tab === 'offers' && (
