...
function AdminApp() {
  ...
  const audienceByCode = new Map(configuredAudiences.map((audience) => [String(audience.code || '').toUpperCase(), audience]));
  const formatAudienceLabel = (code) => {
    const normalizedCode = String(code || '').toUpperCase();
    const audience = audienceByCode.get(normalizedCode);
    return audience?.name ? `${audience.name} (${normalizedCode})` : normalizedCode || 'Grupo não identificado';
  };
