...
async function sendSelected() {
  ...
  const response = await chrome.runtime.sendMessage({ type: 'SEND_COUPONS', coupons: selected });
  setStatus(response?.error || response?.message || 'Enviado. Confira a revisão no painel.'); sendButton.disabled = false;
}
document.querySelector('#save').addEventListener('click', saveSettings); document.querySelector('#scan').addEventListener('click', scanPage); sendButton.addEventListener('click', sendSelected); loadSettings();
